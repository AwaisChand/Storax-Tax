...
class EditPersonalInfoScreen extends StatefulWidget {
  ...
  final PersonalInfoList personalInfoList;
  final List<Dependents> dependent;
...
class _EditPersonalInfoScreenState extends State<EditPersonalInfoScreen> {
  ...
  void initState() {
    ...
    final info = widget.personalInfoList;
    ...
    if (years.contains(currentYear)) {
      selectedYear = info.year ?? currentYear;
      selectedDate = DateTime.now();
    }
    personalInfoNoChange = info.noChange ?? false;
    firstNameController.text = info.firstName ?? '';
    lastNameController.text = info.lastName ?? '';
    homeAddressController.text = info.homeAddress ?? '';
    workPhoneController.text = info.workPhone ?? '';
    emailController.text = info.email ?? '';
    faxController.text = info.fax ?? '';
    homePhoneController.text = info.homePhone ?? '';
    maritalChangeDetailsController.text = info.maritalChangeDetails ?? '';
    commonLawPartnerController.text = info.spouseName ?? '';
    selectedLanguage =
        (info.languageOfCorrespondence ?? "fr").toLowerCase().startsWith("f")
            ? "fr"
            : "en";
    ownsForeignProperty =
        (widget.personalInfoList.ownForeignProperty ?? true) ? "Yes" : "No";

    // selectedLanguage = (info.languageOfCorrespondence ?? "french").capitalize();
    canadianCitizen = (info.canadianCitizen ?? false) ? "Yes" : "No";
    selectedProvince = _mapCodeToProvince(info.provinceOfResidence);
    maritalStatus = _capitalize(info.maritalStatus ?? "Single");
    hasMaritalStatusChanged =
        (info.maritalStatusChanged ?? false) ? "Yes" : "No";
    maritalChangeDate =
        info.maritalChangeDate != null
            ? DateTime.tryParse(info.maritalChangeDate!)
            : null;
    spouseInfo = info.noSpouse ?? false;
    rl19Value = (info.rl19 ?? false) ? "Yes" : "No";
    estate = (info.realEstateDisposition ?? false) ? "Yes" : "No";
    residence = (info.principalResidenceDisposition ?? false) ? "Yes" : "No";
    taxReturn = (info.firstTaxReturn ?? false) ? "Yes" : "No";

    ramq = info.ramqMonths ?? 0;
    pMonths = info.privateInsuranceMonths ?? 0;
    amount = double.tryParse(info.rl19Amount ?? '0.0') ?? 0.0;

    dependents = List.from(widget.dependent);
  }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            Checkbox(
                              value: widget.personalInfoList.noChange,
                              activeColor: AppColors.goldenOrangeColor,
                              onChanged: (bool? value) {
                                FocusScope.of(context).unfocus();

                                setState(() {
                                  widget.personalInfoList.noChange =
                                      value ?? false;
                                });
                              },
                            ),
                            ...
                            Expanded(
                              child: Text(
                                AppLocalizations.of(
                                      context,
                                    )!.translate("personalInfoNoChangeText") ??
                                    '',
                                style: GoogleFonts.poppins(
                                  fontSize: 13,
                                  fontWeight: FontWeight.w500,
                        ...
                        IgnorePointer(
                          ignoring: widget.personalInfoList.noChange ?? false,
                          child: Row(
                            children: [
                              Expanded(
                                child: TextFormField(
                        ...
                        IgnorePointer(
                          ...
                          child: Wrap(
                            spacing: 10,
                            runSpacing: 0,
                            children: List.generate(maritalOptions.length, (
                              index,
                            ) {
                        ...
                        Row(
                          children: [
                            IgnorePointer(
                              ignoring:
                                  widget.personalInfoList.noChange ?? false,
                              child: Checkbox(
                                value: spouseInfo,
                                activeColor: AppColors.goldenOrangeColor,
                                onChanged: (bool? value) {
                                  ...
                                  DependentRow(
                                    ...
                                    onChanged: (dependentDataEdit) {
                                      if (dependents.length <= i) {
                                        dependents.add(dependentDataEdit);
                                      } else {
                                        dependents[i] = dependentDataEdit;
                                      }
                                    },
                                  ),
                        ...
                        AppButton(
                          onPressed: () {
                            // 2️⃣ Capitalize marital status
                            final sanitizedMaritalStatus =
                                maritalStatus.isNotEmpty
                                    ? maritalStatus[0].toUpperCase() +
                                        maritalStatus.substring(1).toLowerCase()
                                    : "Single";

                            // 3️⃣ Format marital change date for MySQL DATE column
                            final formattedMaritalChangeDate =
                                maritalChangeDate != null
                                    ? "${maritalChangeDate!.year.toString().padLeft(4, '0')}-"
                                        "${maritalChangeDate!.month.toString().padLeft(2, '0')}-"
                                        "${maritalChangeDate!.day.toString().padLeft(2, '0')}"
                                    : null;

                            // 4️⃣ Sanitize dependents
                            final sanitizedDependents =
                                dependents.map((d) {
                                  return {
                                    "name": d.name ?? '',
                                    "disability":
                                        d.disability == "1" || d.disability == 1
                                            ? 1
                                            : 0,
                                    "dob": d.dob ?? '',
                                    "citizenship": d.citizenship ?? '',
                                    "months":
                                        int.tryParse(d.months.toString()) ?? 0,
                                    "income":
                                        double.tryParse(d.income.toString()) ??
                                        0.0,
                                  };
                                }).toList();

                            // 5️⃣ Construct final API data
                            Map<String, dynamic> data = {
                              "year": selectedYear,
                              "no_change":
                                  widget.personalInfoList.noChange == true
                                      ? 1
                                      : 0,
                              "first_name": firstNameController.text.trim(),
                              "last_name": lastNameController.text.trim(),
                              "home_address": homeAddressController.text.trim(),
                              "work_phone": workPhoneController.text.trim(),
                              "home_phone": homePhoneController.text.trim(),
                              "fax": faxController.text.trim(),
                              "email": emailController.text.trim(),
                              "language_of_correspondence": sanitizeLanguage(
                                selectedLanguage,
                              ),
                              "canadian_citizen": canadianCitizen == "Yes",
                              "province_of_residence": _mapProvinceToCode(
                                selectedProvince,
                              ),
                              "marital_status": sanitizedMaritalStatus,
                              "marital_status_changed":
                                  hasMaritalStatusChanged == "Yes",
                              "marital_change_date": formattedMaritalChangeDate,
                              "marital_change_details":
                                  maritalChangeDetailsController.text.trim(),
                              "spouse_name":
                                  commonLawPartnerController.text.trim(),
                              "ramq_months": ramq,
                              "private_insurance_months": pMonths,
                              "rl19": rl19Value == "Yes",
                              "rl19_amount": amount,
                              "real_estate_disposition": estate == "Yes",
                            ...
                            print("Sanitized API data: $data");

                            // 6️⃣ Call API
                            taxManager.editPersonalInfoApi(
                              context,
                              data,
                              widget.personalInfoList.id!,
                            );
                          },
                          btnText:
                              AppLocalizations.of(
